var util 		= require('util'),
	...
	function QueryByID(req, user, year, doy, credentials, entries ) {
		...
		var key =  subfolder+"/2011/Pakistan"+"/"
		
		if( bucketList[key] != undefined ) {	
			var artifacts			= bucketList[key]
						
			var s3host				= "https://s3.amazonaws.com/ojo-workshop/ls/2011/Pakistan/"
			var browse_img			= "Pakistan_thn.jpg"
			var topojson_file		= "Pakistan_levels.topojson"
			var topojson_gz_file	= "Pakistan_levels.topojson.gz"
			...
			actions = [
				{ 
					...
					"using": [{
						"@type": 		"as:HttpRequest",
						"method": 		"GET",
						"url": 			Bewit(host+"/products/pop/browse/"+year+"/"+doy),
						"mediaType": 	"html"
					}]
				},
				{
	...
	function check( req, user, d, startTime, endTime, credentials, entries, callback ) {
		...
		QueryByID(req, user, year, doy, credentials, entries)
	}
	...
	function Query(req, user, credentials, host, query, bbox, lat, lon, startTime, endTime, startIndex, itemsPerPage, limit, cb ) {
		...
		function checkAllRequestedDays(next) {
			check( req, user, null, startTime, endTime, credentials, entries )
			next(null)
		}	
