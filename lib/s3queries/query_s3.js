var util 		= require('util'),
	...
	function getGeopixValue(fileName, lat, lng) {
		logger.debug("getGeopixValue", fileName, lat, lng)
	...
	// if not, get it
	//
	Query.prototype.CheckEmptyBucketList = function(bucket, prefix, next) {
	...
	Query.prototype.QueryByID = function(req, user, year, doy, regionKey, prefix, credentials, cb ) {
		...
		function checkEmptyBucket(next) {
			self.CheckEmptyBucketList(bucket, prefix, next)
		}
			...
			function checkFilePresent( subfolder, ftype, mediaType, format, fmt ) {
				if(ftype) {
					...
					try {						
                        ...
                        if( obj == undefined ) {
							console.log(fmt, "not found in ", artifacts)
							return
						}
	...
	//http://localhost:7465/products/gpm/features/Global/2015/245/gpm_1d.20150902.geojson/query?
	//returnGeometry=true
	//&where=1%3D1
	//&outSr=4326
	//&outFields=*
	//&inSr=4326
	//&geometry=%7B%22xmin%22%3A-135%2C%22ymin%22%3A21.94304553343818%2C%22xmax%22%3A-112.5%2C%22ymax%22%3A40.979898069620155%2C%22spatialReference%22%3A%7B%22wkid%22%3A4326%7D%7D
	//&geometryType=esriGeometryEnvelope
	//&spatialRel=esriSpatialRelIntersects
	//&geometryPrecision=5
	//&maxAllowableOffset=0.03805749393203883
	//&f=json
	...
	Query.prototype.Features= function(req,res) {
		...
		function CheckStore( store ) {
			...
			store.within(bbox, function(err, features) {
				...
				//console.log("Sending",featureCollection.features.length )
		...
		lockFile.lock(geoStoreCacheLock, options, function(err) {	
			if( geoStoreCache[prefix] === undefined ) {
				...
				lockFile.unlockSync(geoStoreCacheLock)
				CheckStore(geoStoreCache[prefix])
			}	
		})
	}
	...
	Query.prototype.Export= function(req,res) {
		...
		console.log("Export", prefix, fileName)
		...
		function CreateShapeFile(shpDir, jsonFileName, zipFileName, cb) {
			...
			exec(cmd, function (error, stdout, stderr) {
				console.log("convert", error, stdout, stderr)
				cb(error)
			})
		}	
		...
		if( prefix.indexOf(".shp.zip") > 0) {
			...
			var topofileName 	= path.join(tmp_dir, bucket, subfolder, year, doy, newprefix)
			console.log("export to arcjson from", topofileName)
			...
			DownloadTopojsonFilefromS3andConvertToGeoJSON(topofileName, geojsonfileName, bucket, subfolder, year, doy, newprefix, function(err, geojson) {
				if( !err ) {
					...
					if( !err ) {
			 		...
			 		console.log("Got err from D", err)
			 	}
	 		 	res.sendStatus(err)
			})
		} else {
	...
	Query.prototype.Browse= function(req,res) {
		...
		this.CheckEmptyBucketList(bucket, prefix, function() {
			...
			var json_product		= _.find(artifacts, function(el) { 
							return el.key.indexOf("topojson.gz") > 0
						}).key
					
			var data_url			= s3proxy+json_product || slf.options.geojson
		
			console.log("Browse", slf.options.subfolder, year, doy, prefix, json_product)
			...
			if( slf.options.prefix_map ) {
				var str = "legend."+product_title+"."+prefix
				product_description	= req.gettext(str)
				console.log(str, product_description)
			}
