var util 			= require('util'),
			...
			glob(data_dir + "/*/*/*", options, function (err, files) {
				_.each(files, function(f) {
					...
					if( slf.bucketList[key] != undefined ) {
						...
						logger.debug("added to key", key, name)
					} else {
	...
	Query.prototype.CheckIfProductInBucketList = function(req, key, year, month, day, jday, id, Bewit, regionKey, prefix, multi, next) {
		debug("CheckIfProductInBucketList %s %s %d year: %d month: %d day: %d, jday: %d", key, prefix, multi, year, month, day, jday)
		...
		if( this.bucketList[key] != undefined ) {				
			...
			function CheckIfProductExists(slf, key) {
				...
				if( slf.options.tilejson) {
					mapAction.addHttpGet("tilejson", "tilejson", host+"/products/"+slf.options.subfolder +"/tilejson/"+regionKey+"/"+year+"/"+jday+"/"+file_prefix,	null, "application/json")
				}
				...
				//logger.debug("CheckIfProductInBucketList", JSON.stringify(entry))
				return entry
			}
	...
	Query.prototype.QueryAll = function(req, user, credentials, host, query, bbox, lat, lon, startTime, endTime, startIndex, itemsPerPage, limit, cb ) {
		...
		logger.info("QueryAll tag:", query, tags, this.options.source)
	...
	function sendFile( res, file ) {
		...
		//logger.debug("sendFile", file, ext, mime_type)
		
		if( (basename.indexOf(".topojson.gz") > 0) || (basename.indexOf(".geojson.gz") > 0) ) {
			res.header("Content-Type", "application/json")
			res.header("Content-Encoding", "gzip")
			//logger.debug("sending .topojson application/json gzip", basename)
		} else {
	...
	function DownloadFilefromS3(bucket, subfolder, year, doy, id, cb ) {
		//logger.debug("DownloadFilefromS3", bucket, subfolder, year, doy, id)
		...
		logger.debug("DownloadFilefromS3 s3fileName", s3fileName)
	...
	Query.prototype.S3 = function(req,res) {
		...
		//logger.debug("S3 query regionKey", regionKey)
		...
		if( regionKey != 'tmp') {
			...
			logger.debug("sending tmpfile", fileName)
	...
	Query.prototype.Features= function(req,res) {
		...
		//logger.debug("geometry", JSON.stringify(geometry))
		//logger.debug("bbox", JSON.stringify(bbox))
		//logger.debug("Features", prefix )
		...
		function CheckStore( store ) {
			...
			store.within(bbox, function(err, features) {
				//logger.debug("within", err, features.length)
			...
			function CreateStoreFromGeoJSON(geojson, cb) {
				...
				if( geojson && geojson.features && geojson.features.length>0) {
					async.each(geojson.features, function(f, next) {
						...
						//logger.debug("Store created for", prefix)
		...
		//logger.debug("geoStoreCacheLock", geoStoreCacheLock)
		var options = {
			wait: 		1000,
			retries: 	100
		}
		...
		lockFile.lock(geoStoreCacheLock, options, function(err) {	
			...
			if( geoStoreCache[prefix] === undefined ) {
				//logger.debug("geoStore does not exist", prefix)
				CreateStore(function(err, store) {
					if( !err ) CheckStore(store)
					lockFile.unlockSync(geoStoreCacheLock)
					//logger.debug("geoStore unlocked")
						
				})
			} else {
				//logger.debug("geoStore exists", prefix)
	...
	Query.prototype.Export= function(req,res) {
		...
		logger.info("Export", prefix, fileName)
		...
		function CreateKMLFile( jsonFileName, kmlFileName, cb) {
			try {				
				var cmd = "ogr2ogr -f 'KML' " + kmlFileName + " " + jsonFileName + ";\n"
				exec(cmd, function (error, stdout, stderr) {
					logger.info("convert", error, stdout, stderr)
					cb(error)
				})
			} catch(e) {
				logger.error("Error converting to KML file", e)
				cb(500)
			}	
		}
		...
		} else if( prefix.indexOf(".kml") > 0 ) {			
			var newprefix 			= prefix.replace("kml","geojson")
			var geojsonfileName 	= path.join(tmp_dir, bucket, subfolder, year, doy, newprefix)
			if( !fs.existsSync(geojsonfileName)) {
				var newprefix 		= prefix.replace("kml","topojson.gz")
				var topofileName 	= path.join(tmp_dir, bucket, subfolder, year, doy, newprefix)
				DownloadTopojsonFilefromS3andConvertToGeoJSON(topofileName, geojsonfileName, bucket, subfolder, year, doy, newprefix, function(err, geojson) {
					CreateKMLFile(geojsonfileName, fileName, function(err) {
						if(!err) {
							sendFile(res, fileName)
						} else {
							res.sendStatus(err)
...
//
// Vector Tiling SUpport
//
	var vtCache = {}

	// return tilejson
	Query.prototype.Tilejson = function(req, res) {
		var json = {}
		if (this.options.tilejson) {
			json = this.options.tilejson(req)
		} 
		res.send(json)
	}
	
	//return vector style
	Query.prototype.VTStyle = function(req, res) {
		var json = {}
		if (this.options.vtstyle) {
			json = this.options.vtstyle()
		} 
		res.send(json)	
	}

	function getVectorTiles(geojsonFileName) {
		var baseName = path.basename(geojsonFileName)
	
		if( !vtCache[baseName]) {
			var data 	= fs.readFileSync(geojsonfileName, "utf8")
		...
		return vtCache[baseName].vt
	}

	Query.prototype.VectorTile = function(req, res) {
		var regionKey	= req.params['regionKey']
		var region		= app.config.regions[regionKey]
		var bucket		= region.bucket
		var subfolder	= req.params['subfolder']
		...
		var baseName	= prefix.replace('.'+ext, '')
		
		var key 		= baseName+'_'+z+"_"+x+"_" +y
		var cacheTime 	= 24 * 3600 * 1000	//24 hrs in ms
		
		var tmp_dir 	= app.get("tmp_dir")
		
		var vectorLockFile 	= path.join(tmp_dir, "vectorFile.lock")
		...
		logger.debug("VT", baseName, z, x, y)
		
		function cacheTile(tile) {
			var value = JSON.stringify(tile)
			app.memjs.set(key, value, function(err, val) {
				logger.debug("Caching tile", key, err, val, value.length)
			});	
		}
		
		function cachePbf(pbf) {
			app.memjs.set(key, pbf, function(err, val) {
				logger.debug("Caching Pbf tile", key, err, pbf.length)
			});				
		}
		
		function sendPbf(res, pbf) {
			var cacheTime = 24 * 3600 * 1000	//24 hrs in ms
		...
		function sendTile( res, tile ) {
			if (!tile) {
				res.sendStatus(400)
				app.memjs.set(key, 0, function(err, val) {
					logger.debug("Caching null tile", key, err)
				});	
				return
			}
			var options 		= {}
			options[product] 	= tile
			var buff 			= vtpbf.fromGeojsonVt(options)
			var cacheTime 		= 24 * 3600 * 1000	//24 hrs in ms
	        res.setHeader('Cache-Control', 'public; max-age='+cacheTime);
	        res.setHeader('Content-Encoding', 'deflate');
			res.header("Content-Type", "application/x-protobuf")
			zlib.deflate(buff, function(err, pbf) {
				res.send(pbf);
				cachePbf(pbf)
			});		
		}
		...
		function findTile() {
			if( vtCache[baseName]) {
				vtCache[baseName].updated_at = moment()
				var tileIndex 	= vtCache[baseName].vt
		...
		function getTileFromLocalCache() {
			if(vtCache[baseName]) {
				var tile = findTile()
				return tile
			} else{
				//console.log("Cache not found for", baseName)
				for( var k in vtCache ) {
		...
		app.memjs.get(key, function(err, data) {
			//console.log("memjs get", key, err)
			if( useMemCache && data ) {
				if( data == 0) {
					logger.debug("Found null pbf tile " + key + " in memcache")
					return 	res.sendStatus(400)
				}
				
				logger.debug("Found pbf tile " + key + " in memcache - " + data.length)
				
				return sendPbf(res, data)
				//return sendTile(res, tile)
			} else {
				var tile = getTileFromLocalCache()
				if( tile != -1) {
					sendTile(res, tile)
					logger.debug("Found tile " + key + " in local cache")
				} else {
					logger.debug("tile not found: " + key)
					var options = {
						wait: 		1000,
						retries: 	10
					}
		
					lockFile.lock(vectorLockFile, options, function(err) {
						// one more chance
						var tile = getTileFromLocalCache()
						if( tile != -1) {
							logger.debug("Now tile found: " + key)
							sendTile(res, tile)
							lockFile.unlockSync(vectorLockFile)
							return
						}
						// Need to generate the tiles (LAZY way)
						var newprefix 			= prefix.replace("mvt","geojson")
						var geojsonfileName 	= path.join(tmp_dir, bucket, subfolder, year, doy, newprefix)

						var newprefix 			= prefix.replace("mvt","topojson.gz")
						var topofileName 		= path.join(tmp_dir, bucket, subfolder, year, doy, newprefix)
						...
						DownloadTopojsonFilefromS3andConvertToGeoJSON(topofileName, geojsonfileName, bucket, subfolder, year, doy, newprefix, function(err, geojson) {
							if( !err ) {
								var fid = 1
								for( var f in geojson.features) {
									geojson.features[f].id = fid++
								}
								logger.debug("Create vtCache", baseName, key, z, x, y)
								var tileIndex = geojsonvt(geojson, {debug:0})
								...
								logger.debug("Looking for tile", baseName,z,x,y)
								var tile = findTile(year, doy, baseName, z, x, y )
								sendTile( res, tile )
							} else {
								logger.error("Error DownloadTopojsonFilefromS3andConvertToGeoJSON", err)
						 		res.sendStatus(err)
							}
							lockFile.unlockSync(vectorLockFile)
						})
