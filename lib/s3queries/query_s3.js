var util 			= require('util'),
	...
	Query.prototype.ListObjects = function( bucket, prefix, next) {
		...
		var params = {
			...
			Prefix: 	slf.options.Subfolder ? slf.options.Subfolder(prefix) : slf.options.subfolder,
			MaxKeys: 	1000
		};
	...
	Query.prototype.CheckIfProductInBucketList = function(req, key, year, month, day, jday, id, Bewit, regionKey, prefix, multi, next) {
		...
		if( this.bucketList[key] != undefined ) {				
			...
			function CheckIfProductExists(slf, key) {
				...
				var subfolder = slf.options.Subfolder ? slf.options.Subfolder(prefix) : slf.options.subfolder
				...
				var entryDisplayName	= (slf.options.displayName || prefix)+"_"+ymd
				//console.log("entryDisplayName", entryDisplayName)
					
				var entry 				= as.entry(prefix+"_"+ymd, "geoss:"+ slf.options.product, entryDisplayName)
				...
				var mapAction 		= as.action(req.gettext("actions.map"),"ojo:map")
										.addHttpGet("legend", 	req.gettext("mapinfo.legend"), 	host+"/mapinfo/"+slf.options.subfolder+"/legend?product="+prefix, 	null, "text/html")
										.addHttpGet("style", 	req.gettext("mapinfo.style"), 	host+"/mapinfo/"+slf.options.subfolder+"/style?product="+prefix, 	null, "application/json")
