var util 		= require('util'),
	...
	var options = {
		...
		tags: 			['precip_3hr', 'precipitation', 'rainfall', 'accumulation'],
		minzoom: 		6,
		attributes: [
			{
				"name": "precip",
	...
	function build_text() {
		for( var l in levels ) {
			var legend_entry = "legend.precipitation_gpm.legend."+levels[l]
	...
	options.legend = function(req) {
		...
		html += "  <div class='legend-title'>"+ req.gettext("legend.precipitation_gpm.gpm_3hr")+"</div>"
	...
	options.tilejson = function(req) {
		...
		var url1 				= host + "/products/gpm_3hr/vt/Global/"+ year+"/" + doy+"/{z}/{x}/{y}/" + prefix+".mvt"
		
		var json = {
			"tilejson": 	"1.0.0",
			"name": 		"GPM/IMERG 3hr",
			"description": 	"GPM/IMERG accumulated precipitation for last 3 hours",
			"version": 		"1.0.0",
