...
var util 		= require('util'),
	...
	options.legend = function(req) {
		...
		html += "	   <li><span style='background: " + colors[6] + "'></span>&nbsp;"+ req.gettext("legend.population_count.legend.8900") +"</li>"
		html += "	   <li><span style='background: " + colors[5] + "'></span>&nbsp;"+ req.gettext("legend.population_count.legend.5500") +"</li>"
		html += "	   <li><span style='background: " + colors[4] + "'></span>&nbsp;"+ req.gettext("legend.population_count.legend.3400") +"</li>"
		html += "	   <li><span style='background: " + colors[3] + "'></span>&nbsp;"+ req.gettext("legend.population_count.legend.2100") +"</li>"
		html += "	   <li><span style='background: " + colors[2] + "'></span>&nbsp;"+ req.gettext("legend.population_count.legend.1300") +"</li>"
		html += "	   <li><span style='background: " + colors[1] + "'></span>&nbsp;"+ req.gettext("legend.population_count.legend.800") +"</li>"
		html += "	   <li><span style='background: " + colors[0] + "'></span>&nbsp;"+ req.gettext("legend.population_count.legend.500") +"</li>"
		html += "    </ul>"
		html += "  </div>"
	...
	Query.prototype.LandScanQueryAll= function(req, user, credentials, host, query, bbox, lat, lon, startTime, endTime, startIndex, itemsPerPage, limit, cb ) {
		...
		self.ListObjects(bucket, 'ls', function(err) {
			...
			var s3proxy				= host+'/products/s3/'+regionKey+"/ls/2011/-/"
			
			var browse_img			= "ls.2011_thn.jpg"
			var topojson_file		= "ls.2011.topojson"
			var topojson_gz_file	= "ls.2011.topojson.gz"
						
			var topojson_size, topojson_gz_size
			
			var topojson		= _.find(artifacts, function(el) { return el.key == topojson_file})
			if( topojson ) {
				topojson_size = topojson.size
			} else {
				return cb(err,null)
			}
						
			var topojson_gz		= _.find(artifacts, function(el) { return el.key == topojson_gz_file })
			if( topojson_gz ) {
				topojson_gz_size = topojson_gz.size
			} else {
				return cb(err,null)	
			}
