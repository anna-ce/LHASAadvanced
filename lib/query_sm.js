var util 		= require('util'),
	...
	function QueryByID(req, user, year, doy, credentials, cb ) {
		...
		var key =  "sm" + "/" + date.year() + "/" + doy + "/" + "sm."+date.year()+month+day+".120000.tif"
		...
		app.s3.headObject(params, function(err, data) {
			...
			var s3host				= "https://s3.amazonaws.com/ojo-workshop/sm/2015/079/"
	...
	function checkSM( req, user, d, startTime, endTime, credentials, entries, callback ) {
		time				= endTime.clone()
	...
	function QuerySM(req, user, credentials, host, query, bbox, lat, lon, startTime, endTime, startIndex, itemsPerPage, limit, cb ) {
		//console.log("QuerySM")
		...
		async.each(days, function(d, callback) {
			if( entries.length < limit ) {
				checkSM( req, user, d, startTime, endTime, credentials, entries, callback )
			} else {
				callback(null, null)
			}
		}, function(err) {
	...
	module.exports.QuerySM 		= QuerySM;
