var util 		= require('util'),
	...
	function findLandslideNowcast(regionId, ymd, lat, lon) {		
		...
		try {
			...
			if( fs.existsSync(fileName)) {
				...
				var result 			= tif.LatLng(lat, lon)
				logger.info("findLandslideNowcast result", result)
				return result
			...
			logger.error("findLandslideNowcast Exception", e)
		}
		return 0
	}
	
	function findDailyPrecip(regionId, ymd, lat, lon) {		
		if( regionId == 'd03') dx = "ojo-d3"
		if( regionId == 'd02') dx = "ojo-d2"
			
		try {
			var fileName = "./tmp/"+dx+"/" + ymd + "/trmm_24_"+regionId+"_"+ymd+".tif"
			if( fs.existsSync(fileName)) {
				var tif				= geopix.GEOTIFFFile(fileName)
				var result 			= tif.LatLng(lat, lon)
				logger.info("findDailyPrecip result", result)
				return result
			...
			logger.error("findDailyPrecip Exception", e)
		}
		return 0
	}
	...
	function QueryLocationCast(req, user, credentials, host, query, bbox, lat, lon, startTime, endTime, startIndex, itemsPerPage, limit, cb ) {
		...
		var r1 = findLandslideNowcast(id, ymd, lat, lon) 
		var r2 = findDailyPrecip(id, yesterday, lat, lon) 	// last 24hrs
		
		console.log("landslide/precip results:", r1, r2)
