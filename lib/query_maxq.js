var util 		= require('util'),
	...
	function QueryByID(req, user, year, doy, credentials, cb ) {
		...
		var duration	= 60 * 30
		var id			= "maxq_" + year.toString() + doy
		...
		var key =  "maxq" + "/" + year + "/" + doy + "/" + "maxq."+year+month+day+".120000.tif"
		...
		app.s3.headObject(params, function(err, data) {
			...
			var browse_img_url		= s3host+"maxq."+year+month+day+".120000_thn.jpg"
			var topojson_url		= s3host+"maxq."+year+month+day+".120000_levels.topojson"
			var topojson_file		= s3host+"maxq."+year+month+day+".120000_levels.topojson.gz"
	...
	function Query(req, user, credentials, host, query, bbox, lat, lon, startTime, endTime, startIndex, itemsPerPage, limit, cb ) {
		//console.log("Query MaxQ")
		
		if( query != 'stream_flow') {
