...
function checkTRMM( req, user, ymd, region, credentials, entries, callback ) {
...
function QueryTRMM(req, user, credentials, host, query, bbox, lat, lon, startTime, endTime, startIndex, itemsPerPage, limit, cb ) {
	...
	console.log("QueryTRMM", limit, region.name)
	limit = 2
	...
	while( endTime.isAfter(startTime) || startTime.isSame(endTime)) {
		...
		if(ymds.length >= limit) break;
	}

	entries		= []

	async.each(ymds, function(ymd, callback) {
		checkTRMM( req, user, ymd, region, credentials, entries, callback )
	}, function(err) {
