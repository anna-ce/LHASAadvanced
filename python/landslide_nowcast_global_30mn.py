...
#
# Generates 24hr Forecast Landslide Estimate every 30mn
#
...
def get_early_gpm_files(gis_files, product_name, ymd_org, hour_min):
	global force, verbose
	downloaded_files = []
	...
	for f in gis_files:
		...
		mydir	= os.path.join(config.data_dir, product_name, ymd_org, hour_min)
...
def get_late_gpm_files(gis_files, product_name, ymd_org, hour_min):
	global force, verbose
	downloaded_files = []
...
def process(_dir, files, ymd, hour_min):
	...
	fname_1km_final 	= os.path.join(_dir, "global_landslide_nowcast_30mn.%s.%s00.tif"%(ymd, hour_min))
	...
	topojson_filename 	= os.path.join(_dir, "global_landslide_nowcast_30mn.%s.%s00.topojson" % (ymd, hour_min))
	merge_filename 		= os.path.join(geojsonDir, "global_landslide_nowcast_30mn.%s.%s00.geojson" % (ymd, hour_min))
	attr				= "nowcast"
	...
	sw_osm_image		= os.path.join(_dir, "global_landslide_nowcast_30mn.%s.%s00_thn.jpg" % (ymd,hour_min))
	
	browse_filename		= os.path.join(geojsonDir, "global_browse.%s.%s00.tif" % (ymd,hour_min))
	subset_filename 	= os.path.join(geojsonDir, "global.%s.%s00.small_browse.tif" % (ymd,hour_min))
	transparent			= os.path.join(geojsonDir, "global.%s.%s00.small_browse_transparent.tif" % (ymd,hour_min))
...
if __name__ == '__main__':
	...
	product_name	= "global_landslide_nowcast_30mn"
	s3_folder		= os.path.join(product_name, str(year), doy)
	
	region			= config.regions['global']
	s3_bucket		= region['bucket']

	_dir			= os.path.join(config.data_dir, product_name, ymd, "%02d%02d"%(hour,minutes))
	...
	for i in range(0,1):
		...
		gis_file_day		= "3B-HHR-E.MS.MRG.3IMERG.%d%02d%02d-S%02d%04d-E%02d%04d.%04d.V03E.1day.tif" % (tyear, tmonth, tday, thour, start_minutes, thour, end_minutes, elapsed_minutes)
		gis_file_day_tfw 	= "3B-HHR-E.MS.MRG.3IMERG.%d%02d%02d-S%02d%04d-E%02d%04d.%04d.V03E.1day.tfw" % (tyear, tmonth, tday, thour, start_minutes, thour, end_minutes, elapsed_minutes)

		early_files.append(gis_file_day)
		early_files.append(gis_file_day_tfw)
		#print gis_file_day
	
	get_early_gpm_files(early_files, product_name, ymd, "%02d%02d"%(hour,minutes))
	...
	for i in range(1,7):
		...
		gis_file_day		= "3B-HHR-L.MS.MRG.3IMERG.%d%02d%02d-S%02d%04d-E%02d%04d.%04d.V03E.1day.tif" % (tyear, tmonth, tday, thour, start_minutes, thour, end_minutes, elapsed_minutes)
		gis_file_day_tfw 	= "3B-HHR-L.MS.MRG.3IMERG.%d%02d%02d-S%02d%04d-E%02d%04d.%04d.V03E.1day.tfw" % (tyear, tmonth, tday, thour, start_minutes, thour, end_minutes, elapsed_minutes)

		#print gis_file_day
		late_files.append(gis_file_day)
		late_files.append(gis_file_day_tfw)
		
	get_late_gpm_files(late_files, product_name, ymd, "%02d%02d"%(hour,minutes))
	...
	process(_dir, all_files, ymd, "%02d%02d"%(hour,minutes))
