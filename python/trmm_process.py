...
color_file 			= os.path.join(config.data_dir,"trmm_colors.txt")
...
def process_latest_trmm_region(dx, dt):
	...
	subset_file 		= os.path.join(config.data_dir,"trmm","trmm_3B42RT_%s_%s_subset_4326.tif" % (dt,dx))
	subset_rgb_file 	= os.path.join(config.data_dir,"trmm","trmm_3B42RT_%s_%s_subset_4326_rgb.tif" % (dt,dx))
	resampled_file 		= os.path.join(config.data_dir,"trmm","trmm_3B42RT_%s_%s_subset_4326_1km.tif" % (dt,dx))
	resampled_rgb_file 	= os.path.join(config.data_dir,"trmm","trmm_3B42RT_%s_%s_subset_4326_1km_rgb.tif" % (dt,dx))
	...
	if force or not os.path.exists(resampled_file):
		cmd = "gdalwarp -q -tr 0.008999 0.008999 -r cubicspline -co COMPRESS=LZW -cblend 5 " + subset_file + " " + resampled_file
		if verbose:
			print cmd
		os.system(cmd)
	...
	if force or not os.path.exists(mbtiles_fname):
		...
		json = "{\n"
		json += "  \"name\": \"TRMM 3hr Precipitation - "+ dt + "\",\n"
...
def	process_latest_trmm_file( filename ):
	...
	if force or not os.path.exists(blended_file):
		cmd = "gdalwarp -q -r cubicspline -co COMPRESS=LZW -cblend 5 " + output_file + " " + blended_file
		if verbose:
			print cmd
		os.system(cmd)
	...
	if force or not os.path.exists(mbtiles_fname):			
		...
		metafile = os.path.join(mbtiles_dir, "metadata.json")
		json = "{\n"
		json += "  \"name\": \"TRMM 3hr Precipitation - "+ ym + "\",\n"
		json += "  \"description\": \"TRMM\",\n"
		json += "  \"version\": 1\n"
		json += "}"
		f = open(metafile, "w")
...
#
# Generate the global Accumulation File
#
...
# Subset and resample for specific region
def process_trmm_region( dx ):
	region 	= config.regions[dx]
	bbox	= region['bbox']
	tzoom   = region['tiles-zoom']
    
	subset_file 		= os.path.join(config.data_dir,"trmm", "trmm_24_%s_%s_subset_4326.tif" % (dx,ym))
	subset_rgb_file 	= os.path.join(config.data_dir,"trmm", "trmm_24_%s_%s_subset_4326_rgb.tif" % (dx,ym))
	resampled_file 		= os.path.join(config.data_dir,"trmm", "trmm_24_%s_%s_subset_4326_1km.tif" % (dx,ym))
	resampled_rgb_file 	= os.path.join(config.data_dir,"trmm", "trmm_24_%s_%s_subset_4326_1km_rgb.tif" % (dx,ym))
