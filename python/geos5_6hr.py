#!/usr/bin/env python
#
# GEOS-5 daily precipitation Forecast Product to run every 6 hours
#
...
def get_files(year, hour, mydir, files):
	pool 		= multiprocessing.Pool(processes=processes)
	mstr		= "%02d" % month
	dstr		= "%02d" % day

	ftp_site 		= "ftp.nccs.nasa.gov"
	filepath 		= "fp/forecast/Y%s/M%s/D%s/H%02d" % (year, mstr, dstr, hour)		
...
def process_file( mydir, filename, s3_bucket, s3_folder):
	...
	super_subset_file	= os.path.join(mydir, "geos5_precip_super.%s.%02d0000.tif" % (ymd, hour))
	merge_filename 		= os.path.join(geojsonDir, "geos5_precip.%s.%02d0000.geojson" % (ymd, hour))
	topojson_filename 	= os.path.join(geojsonDir, "..", "geos5_precip.%s.%02d0000.topojson" % (ymd, hour))
	browse_filename 	= os.path.join(geojsonDir, "..", "geos5_precip.%s.%02d0000_browse.tif" % (ymd, hour))
	subset_filename 	= os.path.join(geojsonDir, "..", "geos5_precip.%s.%02d0000_small_browse.tif" % (ymd, hour))
	subset_aux_filename	= os.path.join(geojsonDir, "..", "geos5_precip.%s.%02d0000_small_browse.tif.aux.xml" % (ymd, hour))
	osm_bg_image		= os.path.join(mydir, "../..", "osm_bg.png")
	sw_osm_image		= os.path.join(geojsonDir, "..", "geos5_precip.%s.%02d0000_thn.jpg" % (ymd, hour))
	json_filename		= os.path.join(geojsonDir, "geos5_precip.%s.%02d0000.json" % (ymd, hour))
...
if __name__ == '__main__':
	...
	mydir 		= os.path.join(config.GEOS5_DIR_6hr, ymd, "H%02d"%hour)
	...
	for i in range(start,start+24):	# 0-23
	
		index 	 = i
		if( index > 23 ):
			index -= 24
			filename =  "GEOS.fp.fcst.tavg1_2d_flx_Nx.%s_%02d+%s_%02d30.V01.nc4" %(ymd, hour, ymd1, index)
		else:
			filename =  "GEOS.fp.fcst.tavg1_2d_flx_Nx.%s_%02d+%s_%02d30.V01.nc4" %(ymd, hour, ymd, index)
		
		#print filename
		files.append(filename)
	
	color_file				= os.path.join(basedir, "cluts", "geos5_colors.txt")
	tif_filename			= os.path.join(mydir, "geos5_precip.%s.%02d0000.unflipped.tif" % (ymd, hour))
	flipped_tif_filename	= os.path.join(mydir, "geos5_precip.%s.%02d0000.tif" % (ymd, hour))
	
	if force or not os.path.exists(tif_filename):          
		if verbose:
			print "file not found",   tif_filename
		get_files(str(year), hour, mydir, files)
	...
	rgb_tif_filename	= os.path.join(mydir, "geos5_precip.%s.%02d0000.flipped.rgb.tif" % (ymd,hour))
	...
	s3_folder	= os.path.join("geos5_6hr", str(year), doy)
